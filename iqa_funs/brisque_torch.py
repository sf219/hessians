...
from utils.resize import imresize
...
def _natural_scene_statistics(luma: torch.Tensor, kernel_size: int = 7, sigma: float = 7. / 6) -> torch.Tensor:
    ...
    mu_sq = mu ** 2
    std_2 = F.conv2d(luma ** 2, kernel, padding=kernel_size // 2)
    # this change is needed to avoid nans. The original implementation adds C to std to avoid division by zero. Using abs leads to differentiability issues.
    std = torch.sqrt(std_2 - mu_sq+C)
    luma_nrmlzd = (luma - mu) / (std)
    if (torch.isnan(luma_nrmlzd)).any() == True:
        warnings.warn('Unexpected zero division encountered in BRISQUE.')
    ...
    for shift in shifts:
        ...
        tmp = torch.lgamma(2. / alpha) - (torch.lgamma(1. / alpha) + torch.lgamma(3. / alpha)) / 2
        eta = (sigma_r - sigma_l) * torch.exp(tmp)
