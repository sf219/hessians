...
from compute_q.compute_Q_pytorch import compute_Q_resnet18 as compute_Q
...
for ind_image in range(num_images):
    ...
    Q = compute_Q_obj.sample_q_train(img_org)
    plt.imshow(Q)
    plt.xticks([])
    plt.yticks([])
    plt.colorbar()
    plt.title('Diagonal. Image: ' + str(ind_image))

    plt.subplot(1, 3, 3)
    Q = compute_Q_obj.sample_q_ip_gaussian_train(img_org)
    plt.imshow(Q)
    plt.xticks([])
    plt.yticks([])
    plt.colorbar()
