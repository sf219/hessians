...
for i in range(num_images):
    ...
    plt.plot(bits[:, i], mse_vals_jpeg[:, i], label='JPEG', marker='o')
    plt.plot(bits_nsqjpeg[:, i], mse_vals_nsqjpeg[:, i], label='NSQJPEG', marker='o')
    plt.xlabel('bpp')
    plt.ylabel('PSNR MSE')
    plt.legend()
    plt.subplot(1, 2, 2)
    plt.plot(bits[:, i], emb_vals_jpeg[:, i], label='JPEG', marker='o')
    plt.plot(bits_nsqjpeg[:, i], emb_vals_nsqjpeg[:, i], label='NSQJPEG', marker='o')
    plt.xlabel('bpp')
    plt.ylabel('PSNR EMB')
    plt.legend()
    plt.savefig('sketch/simul_tmps/curves_{}.pdf'.format(i), bbox_inches='tight')
    plt.figure()
    plt.subplot(1, 5, 1)
    ...
    plt.xticks([])
    plt.yticks([])
    plt.savefig('sketch/simul_tmps/images_{}.pdf'.format(i), bbox_inches='tight')
