...
ood_samples = np.zeros((num_images, 5))
for ind_image in range(num_images):
    ...
    for lay in range(5):
        ood = compute_Q_obj.compute_ood(img, lay)
        ood_samples[ind_image, 4-lay] = ood
        print('Layer: ', 4-lay, 'OOD: ', ood)

plt.figure()
# do a box plot with the OOD of each layer
plt.boxplot(ood_samples)
plt.xticks(np.arange(1, 6), ('Block 1', 'Block 2', 'Block 3', 'Block 4', 'Avg. Pool'), fontsize=16)
