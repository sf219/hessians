...
from utils.q_utils_jax import brisque_func, compute_LPIPS_gs
...
mean_conv = np.mean(times_conv)
std_conv = np.std(times_conv)
...
times = np.concatenate((times_hessian_ssim, times_hessian_msssim, times_hessian_brisque, times_hessian_lpips), axis=1)
plt.figure()
plt.boxplot(times)
plt.ylabel('Time (s)', fontsize=16)
# set as xtick the ssim
plt.xticks([1, 2, 3, 4], ['Hessian SSIM', 'Hessian MS-SSIM', 'Hessian BRISQUE', 'Hessian LPIPS'], fontsize=16)
