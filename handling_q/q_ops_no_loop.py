...
class q_ops():
    ...
    def quantize_q(self, new_Q):
        ...
        for n_cwd in n_cwds:
            ...
            for qual_lev in range(nqs):
                lambda_ = 0.85*2**(n_cwd + qual_lev - 1)
                rdos[indis, qual_lev] = SSE + lambda_*rate
    ...
    def choose_ncwd_qlevel(self, qual_level=0):
        rdos = self.rdos[:, qual_level]
        ind = np.argmin(rdos)
