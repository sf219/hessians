...
from handling_q.q_ops import q_ops_resnet
...
compute_Q_obj = compute_Q(true_N=true_N)
q_ops_obj = q_ops_resnet(true_N=true_N, n_cwd=n_cwd, N=N[0])
...
for ind_image in range(num_images):
    ...
    for lay in range(5):
        Q = compute_Q_obj.sample_q_diag_radamacher(img_org, lay)
    ...
    titles = ['Block 1', 'Block 2', 'Block 3', 'Block 4', 'Avg. Pool']
    for lay in range(5):
        Q = compute_Q_obj.sample_q_ip_radamacher(img_org, lay)
        Q = q_ops_obj.normalize_q(Q)
        plt.subplot(2, 3, 6-lay)
        plt.imshow(Q)
        plt.title(titles[4-lay])
