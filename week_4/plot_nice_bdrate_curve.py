...
for i in range(num_images):
    ...
    selection = np.logical_and(mse_vals[:, i] > 28, mse_vals[:, i] < 40)
    
    breakpoint()
    bits_low = np.min(total_bits[selection]) - 0.03
    bits_high = np.max(total_bits[selection]) + 0.03

    selection_nsqjpeg = np.logical_and(total_bits_nsqjpeg >= bits_low, total_bits_nsqjpeg <= bits_high)
    selection = np.logical_and(total_bits >= bits_low, total_bits <= bits_high)

    mse_vals_nsqjpeg = mse_vals_nsqjpeg[selection_nsqjpeg, i]
    mse_vals = mse_vals[selection, i]

    lpips_vals_nsqjpeg = lpips_vals_nsqjpeg[selection_nsqjpeg, i]
    lpips_vals = lpips_vals[selection, i]
    
    ssim_vals_nsqjpeg = ssim_vals_nsqjpeg[selection_nsqjpeg, i]
    ssim_vals = ssim_vals[selection, i]
    
    msssim_vals_nsqjpeg = msssim_vals_nsqjpeg[selection_nsqjpeg, i]
    msssim_vals = msssim_vals[selection, i]

    total_bits = total_bits[selection]
    total_bits_nsqjpeg = total_bits_nsqjpeg[selection_nsqjpeg]

    fs = 15
    plt.figure(figsize=(18, 5))
    plt.subplot(1, 4, 1)
    plt.plot(total_bits, mse_vals, label='JPEG',linewidth=3, marker='o')
    plt.plot(total_bits_nsqjpeg, mse_vals_nsqjpeg, label='MS-SSIM JPEG', linewidth=3, marker='o')
    plt.plot(total_bits_bmshj2018_factorized[3:-1], mse_vals_bmshj2018_factorized[3:-1], label='Ballé et al. 2018', linewidth=3, marker='o')   
    plt.xlabel('bpp', fontsize=fs)
    plt.ylabel('PSNR [dB]', fontsize=fs)
    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.legend(frameon=False, fontsize=fs-1, loc='center right')
    plt.title('PSNR', fontsize=fs)
    plt.subplot(1, 4, 2)
    plt.plot(total_bits, msssim_vals, label='JPEG', linewidth=3, marker='o')
    plt.plot(total_bits_nsqjpeg, msssim_vals_nsqjpeg, label='MS-SSIM JPEG', linewidth=3, marker='o')
    plt.plot(total_bits_bmshj2018_factorized[3:-1], msssim_vals_bmshj2018_factorized[3:-1], label='Ballé et al. 2018', linewidth=3, marker='o')

    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.legend(frameon=False, fontsize=fs-1)
    plt.title('MS-SSIM', fontsize=fs)
    plt.xlabel('bpp', fontsize=fs)
    plt.ylabel(r'$-10\log(1-$MS-SSIM$)$', fontsize=fs)
    plt.subplot(1, 4, 3)
    plt.plot(total_bits, ssim_vals, label='JPEG', linewidth=3, marker='o')
    plt.plot(total_bits_nsqjpeg, ssim_vals_nsqjpeg, label='MS-SSIM JPEG', linewidth=3, marker='o')
    plt.plot(total_bits_bmshj2018_factorized[3:-1], ssim_vals_bmshj2018_factorized[3:-1], label='Ballé et al. 2018', linewidth=3, marker='o')

    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.legend(fontsize=fs)
    plt.legend(frameon=False, fontsize=fs-1)
    plt.title('SSIM', fontsize=fs)
    plt.xlabel('bpp', fontsize=fs)
    plt.ylabel(r'$-10\log(1-$SSIM$)$', fontsize=fs)

    plt.subplot(1, 4, 4)
    plt.plot(total_bits, lpips_vals, label='JPEG', linewidth=3, marker='o')
    plt.plot(total_bits_nsqjpeg, lpips_vals_nsqjpeg, label='MS-SSIM JPEG', linewidth=3, marker='o')
    plt.plot(total_bits_bmshj2018_factorized[3:-1], lpips_vals_bmshj2018_factorized[3:-1], label='Ballé et al. 2018', linewidth=3, marker='o')   
    plt.legend(frameon=False, fontsize=fs)
    plt.title('LPIPS', fontsize=fs)
    plt.xlabel('bpp', fontsize=fs)
    plt.ylabel(r'$-10\log($LPIPS$)$', fontsize=fs-1)
    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.tight_layout()
